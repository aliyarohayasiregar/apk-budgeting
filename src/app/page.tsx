...
export default function Home() {
  ...
  // Show days up to today if current month, otherwise show all days in that month
  const isCurrentMonth = new Date().getMonth() + 1 === selectedMonth && new Date().getFullYear() === selectedYear;
  const maxDay = isCurrentMonth ? new Date().getDate() : new Date(selectedYear, selectedMonth, 0).getDate();
  ...
  if (!userId) return null // Hide until auth check is done
  ...
  return (
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        {/* Page header & Filters */}
        <header className="mb-6 sm:mb-8 animate-fade-in flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 tracking-tight">
              Ringkasan
            </h1>
            <p className="text-stone-500 mt-1 text-sm sm:text-base font-medium">
              Pantau arus kas dan budget Anda
            </p>
          </div>
          ...
          <div className="flex gap-2">
            <select
              ...
              className="pl-4 pr-8 py-2.5 border border-stone-200 rounded-lg focus:ring-2 focus:ring-teal-400 focus:border-teal-400 bg-white text-sm font-semibold text-slate-700 shadow-sm outline-none"
            >
              {months.map((m, i) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-6">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-stone-200 p-5 lg:col-span-2 animate-fade-in">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-base font-bold text-slate-800">Arus Kas</h2>
                  <span className="text-xs font-semibold text-stone-400 bg-stone-100 px-2 py-1 rounded-md">{months[selectedMonth-1]} {selectedYear}</span>
                </div>
